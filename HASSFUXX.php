...
$outPath = "Doxygen/fuxx/";
$srcPath = "Doxygen/website/";

if (! is_dir($outPath)) {
  die("outpath not found\n");
}
...
while ($entry = readdir($dh)) {
  if ($entry == "." or $entry == ".." or $entry == "arangodb.css" or is_dir($entry)) {
    continue;
  }

  $content = file_get_contents($srcPath . $entry);

  $replaced = str_replace("##HASSFUXX##", $content, $template);
  file_put_contents($outPath . $entry, $replaced);
}
closedir($dh);
